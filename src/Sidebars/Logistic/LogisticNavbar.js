...
const LogisticNavbar = ({ open, toggleDrawer, toggleTheme, isDarkMode, setIsLoggedIn }) => {
  const navigate = useNavigate();

  const logout = async () => {
      try {
          const response = await axios.get(
              `${process.env.REACT_APP_API_URL}/api/logistic-member/logout`,
          ...
          if (response.status === 200 || response.status === 204) {
              ...
              sessionStorage.removeItem("logisticMemberSession");
              localStorage.removeItem("logisticMemberSession");
  
              // Clear any cookies if needed
              Cookies.remove("logisticMemberSession", { path: '/' });
