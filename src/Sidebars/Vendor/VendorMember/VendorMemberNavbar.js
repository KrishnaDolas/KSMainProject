...
const VendorMemberNavbar = ({ open, toggleDrawer, toggleTheme, isDarkMode, setIsLoggedIn }) => {
  const navigate = useNavigate();

  const logout = async () => {
    try {
      const response = await axios.post(
        `${process.env.REACT_APP_API_URL}/api/vendor-member/logout`,
        {},
        {
          headers: {
      ...
      if (response.status === 200 || response.status === 204) {
        ...
        Cookies.remove("vendorMemberSession"); // Remove token if stored in cookies
...
export default VendorMemberNavbar;
