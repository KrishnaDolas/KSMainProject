...
const OperationMemberNavbar = ({ open, toggleDrawer, toggleTheme, isDarkMode, setIsLoggedIn }) => {
  const navigate = useNavigate();

  const logout = async () => {
    try {
        const response = await axios.post(
            `${process.env.REACT_APP_API_URL}/api/operational-member/logout`,
            {},
            {
                headers: {
        ...
        if (response.status === 200 || response.status === 204) {
            ...
            sessionStorage.removeItem("operationalMemberSession");
            localStorage.removeItem("operationalMemberSession");

            // Clear any cookies if needed
            Cookies.remove("operationalMemberSession", { path: '/' });
...
export default OperationMemberNavbar;
