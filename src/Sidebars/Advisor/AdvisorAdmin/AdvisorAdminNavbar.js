...
const AdvisorAdminNavbar = ({ open, toggleDrawer, toggleTheme, isDarkMode, setIsLoggedIn }) => {
  const navigate = useNavigate();

  const logout = async () => {
    try {
      const response = await axios.post(
        `${process.env.REACT_APP_API_URL}/api/advisory-admin/logout`,
        {},
        {
          headers: {
      ...
      if (response.status === 200 || response.status === 204) {
        ...
        Cookies.remove("advisoryAdminSession"); // Remove token if stored in cookies
...
export default AdvisorAdminNavbar;
