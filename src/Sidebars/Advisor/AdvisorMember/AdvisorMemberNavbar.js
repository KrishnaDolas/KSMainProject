...
const AdvisorMemberNavbar = ({ open, toggleDrawer, toggleTheme, isDarkMode, setIsLoggedIn }) => {
  const navigate = useNavigate();

  const logout = async () => {
    try {
      const response = await axios.post(
        `${process.env.REACT_APP_API_URL}/api/advisory-member/logout`,
        {},
        {
          headers: {
      ...
      if (response.status === 200 || response.status === 204) {
        ...
        Cookies.remove("advisoryMemberSession"); // Remove token if stored in cookies
...
export default AdvisorMemberNavbar;
