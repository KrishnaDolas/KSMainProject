...
const ProtectedRouteVendorMember = ({ children, isLoggedIn, setIsLoggedIn }) => {
    ...
    useEffect(() => {
        const checkAuth = async () => {
            try {
                await axios.get(`${process.env.REACT_APP_API_URL}/api/vendor-member/dashboard`, {
                ...
                navigate('/VendorMemberLogin'); // Navigate immediately after state update
            }
        };
    ...
    return authenticated ? children : <Navigate to="/VendorAdminLogin" />;
}

export default ProtectedRouteVendorMember;
