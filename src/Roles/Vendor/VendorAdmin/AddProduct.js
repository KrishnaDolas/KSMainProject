...
const AddProduct = () => {
    ...
    useEffect(() => {
        const fetchVendorMembers = async () => {
            try {
                const response = await axios.get(`${process.env.REACT_APP_API_URL}/api/vendor-admin/get-vendor-members`, { withCredentials: true });
                const vendorMembersData = response.data.vendorMembers;
                setVendorMembers(vendorMembersData);
                if (vendorMembersData.length > 0) { // Assuming you want to set initial form values based on the first vendor
                    setFormData((prevData) => ({
                        ...prevData,
                        vendorId: vendorMembersData[0]._id,
                        gstNumber: vendorMembersData[0].gstNo, // Update to use gstNo
                        shopDetails: vendorMembersData[0].shopName, // Update to use shopName
                        brandName: vendorMembersData[0].brandName || "", // Use brandName, default to empty string if unavailable
                    }));
                    setSelectedVendor(vendorMembersData[0]._id); // Set the selected vendor
                }
            } catch (error) {
                console.error("Error fetching vendor members:", error);
                toast.error("Failed to fetch vendor members.");
            }
        };
        fetchVendorMembers();
    }, []);

    // Handle changes when user inputs data into the form
    ...
    const handleVendorChange = (e) => {
        ...
        const selectedVendor = vendorMembers.find(vendor => vendor._id === selectedVendorId);
        if (selectedVendor) {
            setFormData((prevData) => ({
                ...prevData,
                vendorId: selectedVendorId,
                gstNumber: selectedVendor.gstNo, // Fetch GST number
                shopDetails: selectedVendor.shopName, // Fetch shop details
                // brandName: selectedVendor.brandName || "", // Fetch brand name
            }));
        }
    };

    const handleToggleSellChange = (event) => {
        setFormData((prevData) => ({
            ...prevData,
            toggleSell: event.target.checked,
            sellPrice: event.target.checked ? prevData.sellPrice : '', // Reset sellPrice if toggled off
        }));
    };
    ...
    const handleSubmit = async (e) => {
        ...
        const productBasedPrice = parseFloat(formData.productBasedPrice);
        const MRP = parseFloat(formData.MRP);
    
        // Validate required fields
        const errors = [];
        if (!formData.productName) errors.push("Product Name is required.");
        if (!MRP) errors.push("MRP is required.");
        if (!productBasedPrice) errors.push("Product Price is required.");
        if (MRP <= productBasedPrice) errors.push("MRP must be greater than the product's base price.");
        ...
        try {
            ...
            const response = await axios.post(`${process.env.REACT_APP_API_URL}/api/vendor-admin/add-product`, dataToSubmit, { withCredentials: true });
        ...
        <div style={{ display: 'flex', flexDirection: 'row', minHeight: '100vh' }}>
            ...
            <div style={{ padding: '20px', paddingTop: '100px', flex: 1 }}>
                <form onSubmit={handleSubmit}>
                    ...
                    <div className="row mb-3">
                        <div className="col-md-12">
                            <FormControl fullWidth required>
                                <InputLabel>Select Vendor Member</InputLabel>
                                <Select
                                    name="vendorId"
                                    value={selectedVendor}
                                    onChange={handleVendorChange}
                                    label="Select Vendor Member"
                                >
                                    <MenuItem value=""><em>None</em></MenuItem>
                                    {vendorMembers.map((vendor) => (
                                        <MenuItem key={vendor.email} value={vendor._id}>
                                            {vendor.fullName}
                                        </MenuItem>
                                    ))}
                                </Select>
                    ...
                    {/* GST Number, Shop Details, and Brand Name (Read-only) */}
                    <div className="row">
