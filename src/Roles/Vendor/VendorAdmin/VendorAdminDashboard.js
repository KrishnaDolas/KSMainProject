  ...
  const fetchDashboard = async () => {
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_API_URL}/api/vendor-admin/dashboard`,
        {
          withCredentials: true, // Include cookies from the server
        }
      );
      setVendorAdmin(response.data.vendorAdmin);
    } catch (error) {
      if (error.response?.status === 401) {
        setError('Session expired. Please log in again.');
        navigate('/VendorAdminLogin');
      } else if (error.response?.status === 404) {
        setError('Vendor Admin not found.');
      } else {
      ...
      <div style={contentContainerStyle}>
        <div style={bentoGridStyle}>
          ...
          <div style={welcomeTextStyle}>
            ...
            <Typography variant="h6">{vendorAdmin?.fullName || 'User'}</Typography>
          ...
          {loading ? (
            ...
            <Grid container spacing={3}>
              ...
              {vendorAdmin ? (
                  ...
                  <Grid item xs={12} md={6}>
                    <Card style={cardStyle}>
                      <CardContent>
                        ...
                        <Typography variant="body1">{formatDate(vendorAdmin.dob)}</Typography>
                      </CardContent>
                    </Card>
                  </Grid>
                  <Grid item xs={12} md={6}>
                    <Card style={cardStyle}>
                      <CardContent>
                        ...
                        <Typography variant="body1">{vendorAdmin.officialEmail || 'N/A'}</Typography>
                      </CardContent>
                    </Card>
                  </Grid>
                  ...
                  <Grid item xs={12} md={4}>
                    <Card style={cardStyle}>
                      <CardContent>
                        ...
                        <Typography variant="body1">{vendorAdmin.employmentId || 'N/A'}</Typography>
                      </CardContent>
                    </Card>
                  </Grid>
                  <Grid item xs={12} md={4}>
                ...
                <Typography color="error" variant="h6" align="center">
                  vendorAdmin data not available
                </Typography>
              )}
            </Grid>
          )}
