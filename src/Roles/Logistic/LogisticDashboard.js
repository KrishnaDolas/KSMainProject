  ...
  const fetchDashboard = async () => {
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_API_URL}/api/logistic-member/dashboard`,
        {
          withCredentials: true, // Include cookies from the server
        }
      );
      setMember(response.data.logisticMember); // Updated to reflect correct data structure
    } catch (error) {
      if (error.response?.status === 401) {
        setError('Session expired. Please log in again.');
        navigate('/LogisticLogin');
      } else if (error.response?.status === 404) {
        setError('Logistic User not found.');
      } else {
